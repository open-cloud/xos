from core.models import (PlCoreBase, Service, SingletonModel, Slice,
                         SlicePrivilege, User)
...
class Volume(PlCoreBase):
    ...
    name = models.CharField(
        max_length=64, help_text="Human-readable, searchable name of the Volume")

    owner_id = models.ForeignKey(User, verbose_name='Owner')

    description = models.TextField(null=True, blank=True, max_length=130,
                                   help_text="Human-readable description of what this Volume is used for.")
    blocksize = models.PositiveIntegerField(
        help_text="Number of bytes per block.")
    private = models.BooleanField(
        default=True, help_text="Indicates if the Volume is visible to users other than the Volume Owner and Syndicate Administrators.")
    archive = models.BooleanField(
        default=False, help_text="Indicates if this Volume is read-only, and only an Aquisition Gateway owned by the Volume owner (or Syndicate admin) can write to it.")

    cap_read_data = models.BooleanField(
        default=True, help_text="VM can read Volume data")
    cap_write_data = models.BooleanField(
        default=True, help_text="VM can write Volume data")
    cap_host_data = models.BooleanField(
        default=True, help_text="VM can host Volume data")
...
class VolumeAccessRight(PlCoreBase):
    ...
    def __unicode__(
        self): return "%s-%s" % (self.owner_id.email, self.volume.name)
...
class ObserverSecretValue(models.TextField):
    ...
    def is_encrypted(cls, secret_str):
        # all encrypted secrets start with MAGIC_PREFIX, which is NOT
        # base64-encoded
...
class SliceSecret(models.Model):        # NOTE: not a PlCoreBase
    ...
    secret = ObserverSecretValue(
        blank=True, help_text="Shared secret between OpenCloud and this slice's Syndicate daemons.")
    ...
    def select_by_user(user):
        ...
        if user.is_admin:
            ...
            visible_slice_ids = [sp.slice.id for sp in SlicePrivilege.objects.filter(
                user=user, role__role='admin')]
...
class VolumeSlice(PlCoreBase):
    ...
    UG_portnum = models.PositiveIntegerField(
        help_text="User Gateway port.  Any port above 1024 will work, but it must be available slice-wide.", verbose_name="UG port")
    RG_portnum = models.PositiveIntegerField(
        help_text="Replica Gateway port.  Any port above 1024 will work, but it must be available slice-wide.", verbose_name="RG port")

    credentials_blob = models.TextField(
        null=True, blank=True, help_text="Encrypted slice credentials, sealed with the slice secret.")

    def __unicode__(
        self): return "%s-%s" % (self.volume_id.name, self.slice_id.name)
    ...
    def clean(self):
        ...
        if self.UG_portnum < 1025 or self.UG_portnum > 65534:
            raise ValidationError(
                "UG port number must be between 1025 and 65534")

        if self.RG_portnum < 1025 or self.RG_portnum > 65534:
            raise ValidationError(
                "RG port number must be between 1025 and 65534")
    ...
    def save(self, *args, **kw):
        ...
        if slice_secret is None:
            raise syndicatelib.SyndicateObserverError(
                "Failed to get or create slice secret for %s" % self.slice_id.name)
