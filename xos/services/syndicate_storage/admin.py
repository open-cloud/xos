from core.admin import (ReadOnlyAwareAdmin, ServiceAppAdmin,
                        ServiceAttrAsTabInline, SliceInline, XOSTabularInline)
from django import forms
from django.contrib import admin
from django.core.exceptions import ObjectDoesNotExist, ValidationError
from services.syndicate_storage.models import (SyndicateService, Volume,
                                               VolumeAccessRight, VolumeSlice)
...
class SyndicateServiceAdmin(ServiceAppAdmin):
    ...
    fieldsets = [(None, {'fields': ['name', 'enabled', 'versionNumber',
                                    'description', ], 'classes':['suit-tab suit-tab-general']})]
...
class VolumeSliceFormSet(forms.models.BaseInlineFormSet):
    ...
    def verify_unchanged(cls, volume_pk, slice_pk, field_name, new_value):
        vs = None
        try:
            vs = VolumeSlice.objects.get(
                volume_id=volume_pk, slice_id=slice_pk)
    ...
    def clean(self):
        for form in self.forms:
            ...
            rc1, old_peer_port = VolumeSliceFormSet.verify_unchanged(
                volume_pk, slice_pk, 'UG_portnum', cleaned_data['UG_portnum'])
            rc2, old_replicate_port = VolumeSliceFormSet.verify_unchanged(
                volume_pk, slice_pk, 'RG_portnum', cleaned_data['RG_portnum'])

            err1str = ""
            err2str = ""
            if not rc1:
                err1str = "change %s back to %s" % (
                    cleaned_data['UG_portnum'], old_peer_port)
            if not rc2:
                err2str = " and change %s back to %s" % (
                    cleaned_data['RG_portnum'], old_replicate_port)

            if not rc1 or not rc2:
                raise ValidationError(
                    "At this time, port numbers cannot be changed once they are set. Please %s %s" % (err1str, err2str))
...
class VolumeSliceInline(XOSTabularInline):
    ...
    fields = ['volume_id', 'slice_id', 'cap_read_data',
              'cap_write_data', 'cap_host_data', 'UG_portnum', 'RG_portnum']
...
class VolumeAdmin(SyndicateAdmin):
    ...
    detailsFieldList = ['name', 'owner_id', 'description', 'blocksize',
                        'private', 'archive', 'cap_read_data', 'cap_write_data', 'cap_host_data']

    fieldsets = [
        (None, {'fields': detailsFieldList,
                'classes': ['suit-tab suit-tab-general']}),
    ]

    inlines = [VolumeAccessRightInline, VolumeSliceInline]

    user_readonly_fields = ['name', 'owner_id', 'description', 'blocksize',
                            'private', 'archive', 'cap_read_data', 'cap_write_data', 'cap_host_data']
