...
from core.admin import ReadOnlyAwareAdmin, XOSTabularInline
...
class FilteredAdmin(ReadOnlyAwareAdmin):
    ...
    def get_urls(self):
        ...
        my_urls = [
            url(r'^(.+)/filteredlist/$', wrap(self.filtered_changelist_view),
                name="%s_%s_filteredchangelist" % info),
            url(r'^(.+)/embeddedfilteredlist/$', wrap(self.embedded_filtered_changelist_view),
                name="%s_%s_embeddedfilteredchangelist" % info),
            url(r'^(.+)/(.+)/filteredchange$', wrap(self.filtered_change_view),
                name='%s_%s_filteredchange' % info),
            url(r'^(.+)/(.+)/embeddedfilteredchange$', wrap(self.embedded_filtered_change_view),
                name='%s_%s_embeddedfilteredchange' % info),
            url(r'^(.+)/filteredadd/$', wrap(self.filtered_add_view),
                name='%s_%s_filteredadd' % info),
            url(r'^(.+)/embeddedfilteredadd/$', wrap(self.embedded_filtered_add_view),
                name='%s_%s_embeddedfilteredadd' % info),
        ]
        return my_urls + urls
    ...
    def add_extra_context(self, request, extra_context):
        ...
        if getattr(request, "hpcService", None) is not None:
            extra_context["custom_changelist_breadcrumb_url"] = "/admin/hpc/%s/%s/filteredlist/" % (
                self.model._meta.model_name, str(request.hpcService.id))
            if getattr(request, "embedded", False):
                extra_context["custom_add_url"] = "/admin/hpc/%s/%s/embeddedfilteredadd/" % (
                    self.model._meta.model_name, str(request.hpcService.id))
            else:
                extra_context["custom_add_url"] = "/admin/hpc/%s/%s/filteredadd/" % (
                    self.model._meta.model_name, str(request.hpcService.id))
                if len(request.resolver_match.args) > 1:
                    # this is only useful on change views, not changelist views
                    extra_context["custom_delete_url"] = "/admin/hpc/%s/%s/delete/" % (
                        self.model._meta.model_name, request.resolver_match.args[1])
...
class FilteredInline(XOSTabularInline):
    ...
    def get_change_url(self, id):
        ...
        if embedded:
            reverse_path = "admin:%s_embeddedfilteredchange" % (
                self.selflink_model._meta.db_table)
            args = (service_id, id)
        else:
            reverse_path = "admin:%s_filteredchange" % (
                self.selflink_model._meta.db_table)
            args = (service_id, id)

        try:
            url = reverse(reverse_path, args=args,
                          current_app=self.selflink_model._meta.app_label)
